...
class LZW:
    ...
    def findCompressionBenefit(self,compressedcColors):
        maxDictIndex = self.compressionIndex
        RepresentBitsCompressed  = 1
        while(True):
            if (maxDictIndex <= pow(2,RepresentBitsCompressed)):
                break
            RepresentBitsCompressed += 1
        print("Number of bits used to represent one token in encoded stream : ",RepresentBitsCompressed)

        cnt = 0
        for color in compressedcColors:
            for row in color:
                cnt += len(row.split(","))
        encodedStreamLen = cnt*RepresentBitsCompressed
        
        print("Length of encoded Stream :",encodedStreamLen)
        origialStreamLen = (self.height*self.width) * (len(self.red)+len(self.blue)+len(self.green))
        ...
        with open(file_path, "w") as file:
            ...
            file.write("\n")
            file.write("Length of Original Stream : "+str('{:,}'.format(origialStreamLen)))
            file.write("\n")
            file.write("Compression Benefit : "+str('{:,}'.format(origialStreamLen - encodedStreamLen)))

        

    def compress(self):
        self.initCompress()
        print("Height and Width : ",self.height," x ",self.width)
        compressedcColors = []
        ...
        self.findCompressionBenefit(compressedcColors)
